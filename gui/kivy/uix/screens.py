...
from wampum.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from wampum import bitcoin
from wampum.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        import wampum
        try:
            uri = wampum.util.parse_URI(text, self.app.on_pr)
        except:
    ...
    def do_save(self):
        ...
        from wampum.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        ...
        if self.app.wampum_config.get('use_rbf'):
...
class ReceiveScreen(CScreen):
    ...
    def on_address(self, addr):
        req = self.app.wallet.get_payment_request(addr, self.app.wampum_config)
        self.screen.status = ''
        if req:
    ...
    def get_URI(self):
        from wampum.util import create_URI
    ...
    def save_request(self):
        ...
        self.app.wallet.add_payment_request(req, self.app.wampum_config)
...
class AddressScreen(CScreen):
    ...
    def get_card(self, addr, balance, is_used, label):
        ...
        request = self.app.wallet.get_payment_request(addr, self.app.wampum_config)
        if is_used:
            ci.status = _('Used')
        elif request:
    ...
    def do_view(self, obj):
        req = self.app.wallet.get_payment_request(obj.address, self.app.wampum_config)
        if req:
    ...
    def do_delete(self, obj):
        ...
        def cb(result):
            if result:
                self.app.wallet.remove_payment_request(obj.address, self.app.wampum_config)
                self.update()

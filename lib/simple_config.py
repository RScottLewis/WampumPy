...
SYSTEM_CONFIG_PATH = "/etc/wampum.conf"
...
class SimpleConfig(PrintError):
    ...
    def __init__(self, options={}, read_system_config_function=None,
        ...
        self.user_config = {}  # for self.get in wampum_path()
        self.path = self.wampum_path()
    ...
    def wampum_path(self):
        # Read wampum_path from command line / system configuration
        # Otherwise use the user's default data directory.
        path = self.get('wampum_path')
        if path is None:
            path = self.user_dir()
        ...
        self.print_error("wampum directory", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "wampum.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/wampum.conf."""
    result = {}
...
def read_user_config(path):
    """Parse and store the user config settings in wampum.conf into user_config[]."""
    if not path:
        return {}
